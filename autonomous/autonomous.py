...
class FrontAuto(AutonomousStateMachine):
    MODE_NAME = "Front autonomous"
    DEFAULT = True
    # Here magicbot injects components
    hatch_deposit: HatchDepositAligner
    hatch_intake: HatchIntakeAligner
    ...
    def __init__(self):
        super().__init__()
        self.left_front_cargo_bay = Waypoint(
            5.6 - SwerveChassis.LENGTH / 2, 0.2, 0, 0.75
        )
        self.setup_loading_bay = Waypoint(3.3, 3.3, math.pi, 2)
        self.loading_bay = Waypoint(0.2 + SwerveChassis.LENGTH / 2, 3.4, math.pi, 1)
        self.right_front_cargo_bay = Waypoint(
            5 - SwerveChassis.LENGTH / 2, -0.4, 0, 0.5
        )
        self.start_pos = Waypoint(
            1.19 + SwerveChassis.LENGTH / 2, 0 + SwerveChassis.WIDTH / 2, 0, 2
        )
    ...
    def drive_to_cargo_bay(self, initial_call):
        if initial_call:
            ...
            if self.completed_runs == 0:
                waypoints = insert_trapezoidal_waypoints(
                    (self.current_pos, self.left_front_cargo_bay),
                    self.acceleration,
                    self.deceleration,
                )
            elif self.completed_runs == 1:
